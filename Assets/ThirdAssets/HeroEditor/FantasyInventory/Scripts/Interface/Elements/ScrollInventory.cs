...
namespace Assets.HeroEditor.FantasyInventory.Scripts.Interface.Elements
{
    ...
    public class ScrollInventory : ItemContainer
    {
        ...
        public void Refresh(Item selected, bool force)
		{
            ...
            if (!force && _initialized && _hash == ComputeHash(items).GetHashCode())
            {
			...
			_initialized = true;
		    _hash = ComputeHash(items).GetHashCode();
        }
        ...
        private static string ComputeHash(List<Item> items)
        {
            return  string.Join("/", items.Select(i => i.Hash + "/" + i.Count));
        }
    }
}